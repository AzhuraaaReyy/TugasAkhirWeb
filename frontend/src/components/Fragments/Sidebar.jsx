  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 py-6 space-y-2 overflow-y-auto -mt-2">
        {menu.map((item) => {
          const isActive = activeLink ? item.link === activeLink : false;

          return (
            <NavLink key={item.id} to={item.link} className="block">
              <div
              ...
              >
                ...
                <span
                  ...
                  ${isActive ? "text-emerald-600" : "text-emerald-600 group-hover:text-white"}`}
                >
                  {item.icon}
                </span>
                {open && (
