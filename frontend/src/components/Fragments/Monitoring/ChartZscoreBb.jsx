...
export default function ChartZscoreBb({
  ...
  const chartData = data.map((item) => ({
    ...item,
    zscore: item[dataKey], // Normalisasi key agar tooltip konsisten
    medianValue: 0,
    minus2Value: -2,
    minus3Value: -3,
  }));
    ...
    <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm w-full">
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-400">
          Grafik Standar WHO: Status Berat Badan menurut Umur
        </p>
      </div>
            ...
            {data.map((item, idx) => {
              ...
              const getStatusColor = (status) => {
                ...
                if (
                  ...
                  s.includes("severely underweight")
                ) {
                  return "bg-red-500 shadow-red-200";
                }
                if (
                  s.includes("berat badan kurang") ||
